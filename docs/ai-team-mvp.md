...
npm run ai:refine -- builder "follow-up note"
npm run ai:route
...
7. For design work that touches implementation, capture screenshot evidence before final design review.
8. Run `npm run ai:route` again and continue until done.
9. If follow-up feedback arrives after completion, reopen the run with `npm run ai:refine -- <agent> <note>`.
